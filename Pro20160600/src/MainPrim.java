...
public class MainPrim {
	...
	private static int runPrim(Edge initEdge) {
		...
		Deque<Integer> waitNodes = new ArrayDeque<Integer>();
		...
		waitNodes.add(initEdge.source);

		int max = Integer.MIN_VALUE;
		while (!waitNodes.isEmpty()) {
			int currNode = waitNodes.poll();
			visited[currNode] = true;

			// Add candidates from current node
			for (Edge edge : nodeList.get(currNode)) {
			...
			while (!candiateEdges.isEmpty()) {
				...
				if (!visited[candidateEdge.target]) {
					waitNodes.add(candidateEdge.target);
